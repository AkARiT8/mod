    ...
    public static void load(BlockedItemsManager manager) {
        ...
        try (InputStreamReader reader =
                     new InputStreamReader(
                             Objects.requireNonNull(BlockedItemsLoader.class
                                     .getResourceAsStream(
                                             "/assets/hellmod/blockedRecipes/blocked_items.json"
                                     ))
                     )) {

            Type listType =
                    new TypeToken<List<BlockedItemData>>() {}.getType();

            List<BlockedItemData> list =
                    new Gson().fromJson(reader, listType);

            for (BlockedItemData data : list) {

                Identifier itemId =
                        new Identifier(data.getItem());
